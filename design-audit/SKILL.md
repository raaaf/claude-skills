...
2. **Hallucination validator** (deterministic, every finding and elevation). A finding's path and
   line are audited-repo content and never enter a command line: write them to a file with the
   Write tool, one `path<TAB>line` per row, at `{AUDIT_TMP}/locations.tsv` (literal path from Phase
   0), then run the script on that file. It rejects non-integer lines, absolute or `..` paths, missing
   files and out-of-range lines, and prints one line per pair. (An inline template that substituted
...
`### Discarded by fixer` is appended to this report after the fix wave (Phase 6 step 5), not written
here: one line per finding the fix wave discarded, `- [Severity][Dimension] file:line: discarded by
fixer: {reason}`.
...
The in-progress marker stays claimed through the fix wave and is released once in Phase 7; Phase 6
dispatches fixers, and a released marker there would leave them unprotected.

## Phase 5: Fix set

Every Defect in the report (all severities, `CONFIRMED` and `UNCERTAIN`) and every Elevation entry
...
**Run log (fires here — every run reaches Phase 7 regardless of what the fix wave did):**

```bash
for c in "$(dirname "${CLAUDE_SKILL_DIR:-/nonexistent}")/audit/bin/lib-orchestrator.sh" "$HOME/.claude/skills/audit/bin/lib-orchestrator.sh"; do [ -f "$c" ] && { . "$c"; break; }; done   # fresh shell per block: source the lib again
orch_run_log --skill design-audit --outcome "{fixed|reported_only}" \
  --counts "critical={N},important={N},minor={N},elevation_offered={N},discarded={N},fixed={N}"

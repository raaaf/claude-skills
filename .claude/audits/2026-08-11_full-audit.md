...
All but one were closed on 2026-08-11 in follow-up work after the audit. Status marked per item.

1. RESOLVED (fix loop extracted to `references/fix-loop.md`; now 433 and 478). ~~**`audit/SKILL.md` is 585 lines and `full-audit/SKILL.md` 532**, both well over the repo's own 500-line convention, and both grew further during the post-audit hardening (PARTIAL handling, coverage-gap routing). Suggested split exists (extract Step E's fix-wave rules to `references/fix-loop.md`) but it moves contract-bearing text, so it is a deliberate decision rather than a mechanical fix. This is now the largest un-actioned convention violation in the repo and it grows every time the loop gains a rule.
2. RESOLVED (`performance-2026.md` created, `accessibility-2026.md` too, agents rewired). ~~**`code-quality.md` (514) and `performance.md` (513) exceed 500 lines.** Convention says split into a `{name}-2026.md` continuation; `performance-2026.md` does not exist yet.
3. RESOLVED (15 files, fence-aware extraction). ~~**13 guideline files over 100 lines have no table of contents**, against the repo convention. Mechanical but high-churn; not done unprompted.
4. DOCUMENTED, by design. **`feature-audit/bin/run-tests.sh` and `perf-measure.sh` `eval` repo-supplied commands** (`test-command:` from FEATURE_AUDIT.md, `perf-measure:` from `.claude/audit-guidelines.md`). This is the documented, intended design, but it means opening a hostile repo and running the skill executes its commands. Worth an explicit one-time confirmation per repo.
5. STILL OPEN. **The push-gate marker `/tmp/claude-audit-passed-<md5 cwd>` is world-creatable and predictable**, so any local process can pre-forge a pass for 30 minutes. Hardening it touches `audit/SKILL.md`, `ship/SKILL.md` and `~/.claude/settings.json` together, and CLAUDE.md documents two hash conventions that must not be mixed, so this needs a coordinated change.
6. RESOLVED (kept, but now checks `git check-ignore`, warns on tracked files, announces every outcome). ~~**`cache-write.sh` and `patterns-store.sh` append to the audited project's `.gitignore`** without asking. Helpful, but it is an audit tool mutating the repo it is auditing.
7. RESOLVED (set to `mandatory`; the one Critical row carries its own severity). ~~**`data-migrations.md` declares `priority: non_negotiable`** (which anchors findings to Critical) while its own checklist rates most violations Important.
8. RESOLVED (`scope-extensions:` project override; this repo declares `md`, scope 96 to 223 files). ~~**Markdown as source is still invisible to the scope collection.** The globs now cover shell scripts (added post-audit), but not `.md` outside a `prompts/` directory. For most projects that is right: Markdown is documentation, and docs drift is checked by the docs_sync dimension. For THIS repo it is wrong, because the orchestrators, agent definitions and guidelines ARE Markdown and are the actual executable source. That is why this run's scope had to be built by hand. Options: a project-level scope override alongside the existing `.claude/audit-guidelines.md` mechanism, or a documented `SOURCE_GLOBS` env. Not decided unilaterally, since it changes what every audit on every project considers source.
9. RESOLVED (documented as the third eval site; deploy now gated by an explicit confirmation). ~~**`ship/SKILL.md` evals `test-command:` and `deploy-command:`** read from the audited repo's `.claude/ship.md`. Same class as the two documented eval sites in `feature-audit` and `perf-measure`, but not recorded in CLAUDE.md's list of them, and this skill also deploys to production. Either document it alongside the other two or gate it on a per-repo confirmation.

...
**The required set is fixed, not "every key present."** `status-line.sh` checks three specific keys — `cross_ref`, `log`, `issues`, the same three shown in the Format example above — individually, and reports `post_phases=done` only once each of them carries a witnessed `done:<witness>` or `skipped:<reason>`. Any other key on the `post-phases:` line (a typo, a rename, a leftover experimental phase) is read and then ignored: it counts toward nothing, neither completion nor a block. This closes a loophole in the original wording, which said "done" once "ALL keys of the `post-phases:` line are witnessed" — that phrasing itself was satisfiable by dropping or misspelling a key, since a key that isn't on the line can't be unwitnessed.

**Witness matching is anchored to the start of the value.** A value merely CONTAINING `done:` or `skipped:` somewhere no longer counts; it has to begin with one of those prefixes. This closes a reproduced bypass: `post-phases: cross_ref=pending-see-log=done:1` — a value that starts with `pending-see-log=...` and only happens to contain `done:1` further in — used to satisfy the old, unanchored check. Anchoring the match to the start of the value means the phase has to actually be `done` or `skipped`, not just mention the word somewhere downstream.

**A witness or skip reason may contain spaces**, and `full-audit/SKILL.md` allows exactly that (`skipped:<free-text reason>`). The parser splits the `post-phases:` line on whitespace, so a multi-word reason becomes several tokens; only the first token of a phase is shaped like `key=value` and gets read for that key, the words after it are parsed too but discarded unless they themselves happen to form `key=value` for a different required key. Practical consequence: `cross_ref=skipped:effort was low` still satisfies `cross_ref` (the first token, `cross_ref=skipped:effort`, already matches the anchored `skipped:` pattern), and `was`/`low` are simply dropped — they never leak into another phase's value. Write a readable reason, but don't expect the parser to see past its first word.
...
`post_phases=done` only once the three required keys (`cross_ref`, `log`, `issues`) are each witnessed — `=done:<witness>` or `=skipped:<reason>`, matched anchored to the start of the value (see "Post-Phase Witnesses" above). A bare `=done` with no witness does NOT count, and a key outside the required set is read but never counted. Missing file → zero line. The orchestrator decides completion ONLY from the line of the current turn, never from memory.

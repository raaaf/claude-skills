...
This template is passed to the collapsed workers (w1-code, 2-security, w3-frontend, w4-content — each covering the dimensions in its briefing), for both /audit and /full-audit. Other subagents — finding-verifier, fix-verifier, cross-reference, fix-agent, triage, learning-agent — are dispatched from `SKILL.md` with only named fields, not this file, and carry their own copies of the rules that apply to them. Placeholders are replaced by the audit skill.
...
means the value from that file. The read counts against your tool-call budget — it is part of the ~12-call reserve in the
`BATCH_MAX` derivation (`full-audit/references/scope-context-batching.md`) — but NOT against the
/audit section's "max 8 source files" allowance: that allowance counts source files you choose to
open, and the wave-shared file is briefing, not source. Only fields your briefing states directly
...
- **Deliver something, always, and inside your budget.** You have at most 30 tool calls (raised
  from 20 when the dimension workers were collapsed: a multi-dimension worker reads up to 5
  dimension modules (`w3-frontend.md` reads five, `w1-code.md`/`w4-content.md` three) and up to 5 guidelines on top of its files — see `w1-code.md`/`w3-frontend.md`/
  `w4-content.md`; `BATCH_MAX` in `full-audit/references/scope-context-batching.md` is derived
  from this number, the two move together). When you reach that, stop investigating and send the report you have, marking which hotspots you did not get to. A partial report beats silence: there is no timeout on you, so an agent that goes quiet without reporting stalls the whole round until the orchestrator notices and re-prompts (four manual nudges were needed in one run on 2026-08-06). "I found nothing yet" is a valid report; no report is not.
- **Declare uncovered scope, don't just imply it.** This does not relax the coverage requirement above — it only makes a gap visible when one happens. If you stop before covering your whole assignment, end your reply with `COVERAGE: partial | not read: {file1}, {file2}, ...` naming every skipped file/hotspot by path. If you covered everything — including when the reply is "Keine Findings." — end with `COVERAGE: full`. Without this line a stopped-early pass looks identical to a clean one.
...
- **Resource-shaped findings carry the measured number.** A finding about I/O cost, payload or asset size, duplicated lines, or text truncation proposed at `Important` or higher states the quantity you measured (KB, line count, characters vs. available width) in the finding text. Severity without the number gets re-derived by the verifier from scratch, which is the expensive path (learning 2026-08-11).
- **A file reference is a file path, not a type name.** `file:line` names the file the code is in (`Sources/App/QueueRow.swift:30`), never the struct/class (`QueueRow:30`). Two agents in one run reported type names, and the line validator dropped both findings.
- **A denied file or tool permission is reported as a blocker, never worked around.** State what you could not read or run, what you could still establish deterministically without it, and stop there. Do not guess at the content and never invent a value to fill the gap.
- **The dimension tag is one of exactly 12 ids**, spelled like this: `architecture`, `security`, `performance`, `code_quality`, `seo`, `a11y`, `typography`, `ui_design`, `ux`, `animation`, `docs_sync`, `copy`. No aliases (`accessibility`, `A11Y`, `Code Quality`, `docs`): free variants have broken the top-category metric across runs since 2026-08-06.
...
- **Conversation-level trust boundary is its own finding category (security agent, round 1):** when a model tool-input can bypass a safety rule (allergy/consent/override gates), treat it as a distinct category and check ALL variants in ONE pass — word-boundary vs. prefix/substring matching, real yes/no assent vs. mere term presence (negations, -frei/-los suffixes), and scope (a confirmed single term must not exempt its whole trigger class). Do not stop at the first hit; one audit found all three gaps in the same feature spread over three rounds. Checklist: security guideline section XVIII.
...
- **Read code on demand:** If a hotspot alone isn't enough (e.g. consistency check against an existing component), read the file in question with the Read tool. Max 8 source files per audit run (raised from 5 with the worker collapse — one worker now covers several dimensions).
...
- Do NOT re-report findings already decided against in a previous round (refuted or discarded, reason given): {BEREITS_VERWORFEN}
...
Declare uncovered scope, don't just imply it: this does not relax the read-everything rule above, it only makes a gap visible when one happens. If you stop before reading every file in {BATCH_DATEILISTE}, end your reply with `COVERAGE: partial | not read: {file1}, {file2}, ...` naming every unread file by path. If you read everything — including when the reply is "Keine Findings." — end with `COVERAGE: full`. Without this line a stopped-early pass looks identical to a clean one.

...
- **Prevent, do not just detect (2026-08-13, third documented instance).** In a php-cs-fixer project, make EVERY PHP edit via Bash, not only the ones with `\`-qualified classes: the hook also rewrites bracket spacing and can insert `declare(strict_types=1)`, and neither is visible in the test run. Immediately after each PHP edit, run the project's own linter on that one file (`vendor/bin/phpcs <file>` or `composer lint` when it accepts a path) and compare with `git show HEAD:<path>`. A diff hunk you did not write, or a phpcs error that was not there at HEAD, is your regression to revert before you report APPLIED. Detect-and-revert after the round caught the damage three times; running the linter per edit is what stops it.
...
3. **Both sides of a pair:** before APPLIED, grep the sister side of whatever you just touched: the other branch of an if/else or ternary, the analogous markup variant (light/dark, mobile/desktop, singular/plural), or a sibling element that displays the same value your fix changed or hid. Fixing one side while its twin still shows the old state produced two same-shaped self-inflicted regressions in a single run (2026-08). If the sister side exists and contradicts the fix, adjust it as part of the fix — an edit that leaves the pair inconsistent is not done.
4. **A reset you add must name what it can destroy.** If the fix introduces or widens anything that CLEARS state on a trigger you do not control — a timer, a scheduled wake-up, a `scenePhase`/lifecycle observer, a notification handler, a day or session rollover — you do not get to report APPLIED until you have written down which in-progress user state exists on the affected screen and what your reset does to it. Enumerate at least: unsaved input in a field, an edit session on existing content, an open sheet or modal, a media/story playback in progress. For each, state destroyed or harmless with a reason. Anything destroyed gets the reset GATED behind it, and the gate must DEFER rather than skip: a reset that is silently dropped reintroduces the original bug in a narrower form, so name where the held-back work eventually runs.

   This is not hypothetical and it is not rare. On 2026-08-13 a fix for a day-boundary bug (the app kept yesterday's state when left foregrounded past midnight) added a midnight timer that could fire mid-draft, clearing the composer and misdating the entry — strictly worse than the bug it fixed, and caught only by the fix-verifier. The finding you are fixing describes the state that is WRONG; this check asks about the state that is RIGHT and that you are about to throw away. See `guidelines/native-mobile.md` section XIV.

5. **Any comment or docblock you write must obey the target project's own prose rules.** Before
   writing an explanatory comment, docblock, or commit-adjacent text as part of a fix, check it
   against `PROJECT_CONTEXT`/`CLAUDE.md` prose rules the same way you would check code style —
   most commonly a ban on em-dashes (`—`), German umlaut-substitutes (`ae`/`oe`/`ue` instead of real
   `ä`/`ö`/`ü`), or emojis. These rules apply to text YOU generate, not only to the diff you are
   auditing: a self-check on the audited code that ignores the audit's own freshly-written prose is
   incomplete. Real incident (2026-08-18): fix-wave docblocks generated by the orchestrator itself
   used em-dashes twice, violating the exact CLAUDE.md rule that same run's diff was being checked
   against.
6. **A comment you write may only claim what the diff shows.** If your fix adds or edits an explanatory comment that asserts a RESULT — "removed", "checked", "migrated", "consolidated", "now covers all X" — the assertion must be true of the code as it stands after your edit, not of the intent behind it. Scope it to what you actually did: "checked for `.motion`" rather than "all tiles verified", "three of the four call sites" rather than "consolidated".

   Two findings in one run (2026-08-13) were comments left by EARLIER audit fixes: one claimed an ambient 0.65s animation had been removed while it was still in the file, one claimed a 3:1 contrast check across eight colours when only one had been checked. Both read as documentation and functioned as misinformation, and both cost a later audit the time to rediscover the underlying defect. A comment that documents an intention instead of an outcome is worse than no comment: it actively suppresses the next finding.
7. **Symbols and thresholds:** every new symbol you reference is imported (or fully qualified) in the file that uses it, checked by re-reading the file, not by memory of the edit; every new boundary or threshold constant (a limit, a cutoff date, a size, a retry count) carries a one-line comment that names where the value comes from. Both E.5 patch cases of one run (2026-09-03) were exactly these: a missing import and an undocumented boundary.
8. **A corrected fact is corrected everywhere.** If the fix changes a factual claim (a count, a path, a version, a rule stated in prose) that other files repeat or reason from, grep the repo for the core phrase before reporting done and fix every repeat site in the same edit, or list the ones you could not touch under `remaining:`. A fact fixed in one file and left standing in three others is the next audit's docs_sync finding (learning 2026-08-05).
9. **Interaction and feedback you add is subject to the platform rules, not only the finding.** A fix that adds press feedback, a gesture, a hover reveal, or a toggle follows `guidelines/native-mobile.md` section XVI (no `DragGesture(minimumDistance: 0)` inside a `ScrollView` child for press feedback, no hover-only actions) and `guidelines/accessibility.md` for web. This case was written by a fix agent, not found by a worker, so a guideline the workers read is one round too late; check it here.
10. **Text you put into an Alpine/Livewire attribute is code.** Inside `x-on:*`, `x-data`, `x-bind:*` or `wire:*` attribute values, a `'{{ __('key') }}'` interpolation breaks as soon as the translated value contains an apostrophe (`you'll`), exactly like the literal `"` rule above. Use `@js(__('key'))` / `Js::from()` or move the string out of the attribute; after the edit, grep every lang value you introduced or referenced for `'` (incident 2026-08-27: a fix agent introduced `you'll` into an interpolated EN string despite the project's CLAUDE.md rule).
...
- No touching files outside your assignment, by any means. If a fix needs a file another agent owns, do not edit it: report it as blocked and let the orchestrator sequence it. This includes references: do not write code that calls a translation key, route, config value or class that does not exist yet and lives in another file. Report `FIX_RESULT=PARTIAL` with the exact key and text the other file needs instead (2026-09-24: a fixer called `__('quoting.error.accept_sync_failed')` without the key existing, the toast would have shown the raw key).
- **A breakpoint problem gets a breakpoint-scoped fix.** When a finding is about one viewport (e.g. iOS zoom on inputs under 16px on mobile), scope the change to that viewport (`max-sm:`, `sm:`) instead of removing the utility for every size; the desktop layout was deliberate (2026-09-24: removing `!text-xs` globally enlarged a compact desktop widget).
...
- **Never cache an authorization or permission decision.** Do not put the result of an ownership check, policy call, `can()`/`authorize()` or role lookup into a static property, a memoized field, `Cache::remember` or any other store that outlives the current request or crosses users. Authorize per request against live data. A cached "allowed" survives a revoked permission or a transferred record, and a cache keyed without the user id leaks one user's permission to another.
...
- **A test that cannot run is not a green test.** If the test command fails for environment reasons (no `APP_KEY`/`.env` in your shell, a sandbox denial, a DB role error, a missing binary), do not retry with hand-made env values and do not report `APPLIED` on the strength of reading the code. Report `FIX_RESULT=APPLIED | ... | TEST_ENV_BLOCKED: {reason}` so the orchestrator runs the test itself before E.5; 8 of 26 dispatches in one run (2026-08-31) hit this and one verifier silently downgraded to a static verdict.
- **Never run `php artisan migrate`, `migrate:fresh`, or `migrate:rollback` against the test database by hand.** The test run itself migrates via `RefreshDatabase`/`LazilyRefreshDatabase` — a hand migration under a different DB role produced "must be owner of table" failures in a sibling agent's run (2026-09-02). If a fixture or DB state looks broken, report it instead of migrating.

...
Evaluating the script outputs (diff-size gate OK/LARGE/HUGE, pre-checks for secrets/lockfile/binary artifacts, deriving `ALLE_DATEIEN`/`FRONTEND_DATEIEN`/`UNIFIED_DIFF`/`SUPPRESSIONS`/`PROJECT_CONTEXT`, the mandatory audit-context check, and the deterministic-check result table): `references/scope-and-pre-checks.md`. A HUGE diff is not always a hard block: the delta-scope carve-out and the same-HEAD multi-slice path (one W1/W2/W3 set per slice, one shared W4) are both named there as alternatives — check their criteria before aborting.

...
A test that pins user-visible text is only as good as what it reads. Asserting against the source declaration — a translation key, a seed constant, an enum raw value — instead of the resolved, rendered output (localization catalog output, template render, computed display property) proves nothing about what the user actually sees. A complete content rewrite can ship live while the test stays green, because the test and the UI read from two different places. Same failure mode as the color-token check in `ui-visual-design.md:308`: resolve the actual value behind the name/key, never trust the label.

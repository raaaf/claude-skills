...
**Step 0.5: verify the recurrence feed was fed (mandatory, before dispatching the learning agent).** The per-verdict `patterns-store.sh recur` duty lives in `SKILL.md` Phase 2, immediately after the `find.js` verdicts are read and BEFORE the fix decision, because a run with `AUDIT_FIX_SCOPE=none` has no fix wave at all, and a fixer can discard a finding, so a duty hanging off the fix wave would miss those findings. It is what populates `patterns.json`; the learning agent only reads it. On 2026-08-26 a run confirmed 10 findings and wrote 0 store entries, and nothing noticed until the retro. Corrected on 2026-09-18: this paragraph, and `agents/learning-agent.md`, had claimed the duty sat in `workflows/fix.js` and `agents/fix-agent.md`; it was in neither file, had never been anywhere, and 192 audit logs had produced 36 store entries. Check it here, deterministically:
...
**The feed hangs off the verdicts, not off the fixes.** Phase 2 of `SKILL.md` writes the pattern of every `CONFIRMED` finding to a file and calls `orch_patterns_from_file recur <file>` in a sourced block, before any fix decision is taken, so the count is identical whether the run fixes everything, only `Critical`, or nothing at all. That placement is the correction of 2026-09-18: hanging the duty off the fix wave meant it never fired for a `find only` run, never for a `Minor` at any scope, and never when the orchestrator applied fixes by hand (as on 2026-09-15, twice), and Step 0.5 then found `FED_N=0` for a run with 30 confirmed findings. A pattern is finding text and never goes on a command line. The back-fill below is the repair for a run that still arrives here unfed, not a second supported path.
...
- merge `SUPPRESSIONS_TO_ADD` into `.claude/audits/suppressions.json`. **If the file does not exist, create it first** (`{"suppressions": []}`) — any audit run, log, or finding that references a suppression MUST leave a valid `suppressions.json` behind; a dangling reference without the file is an orchestrator bug. **Dedup rule:** run the pattern of every new suppression through `bash "$AUDIT_BIN/normalize-suppression.sh"`, same normalization for existing suppressions. If both produce the same key → keep the existing one, discard the new one. This way "[Security] LIKE injection in scope" and "Like-wildcard injection (security)" are recognized as the same. **Expiry (mandatory on decision reasons):** a suppression whose `reason` is a decision or tradeoff ("accepted risk", "by design", "deliberate") is written with `"expires"` set to one year from today (`date -v+1y +%Y-%m-%d` on macOS), because nothing else in the pipeline ever re-examines it; `scope-and-pre-checks.md` drops the pattern once that date passes and the finding surfaces again for one run. A factual-claim reason ("unused", "no callers") gets no `expires`: it is re-validated on every run already, and a second mechanism would only add noise. **Consent gate (mandatory):** after dedup, present the remaining genuinely-new patterns to the user (chat, or `AskUserQuestion` when available) and write only the ones they approve — `SKILL.md`'s "Allowed orchestrator edits" rule lists `suppressions.json` explicitly as "(with user consent)"; a suppression is LLM-proposed and permanently silences a class of future finding, so it does not get written unasked.

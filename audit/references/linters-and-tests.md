...
**swiftlint output: never truncate before separating errors from warnings.** Count the `error:` lines FIRST and print every one in full, then summarise warnings. Never `head -N` the combined output before that split. Reference case (2026-09-05/06): two `type_body_length` errors were cut by `head -25`, reported as "0 errors", and shipped in TestFlight Build 144.
...
**`#if DEBUG` boundary moved → Step 3c also builds Release.** If the diff adds, removes, or moves an `#if DEBUG` boundary, or makes a file that used to compile DEBUG-only compile in Release too, the normal Debug-configuration test run does not catch a symbol that only exists under DEBUG. Run, in addition to the diff-scoped test suite: `xcodebuild -project <proj> -scheme <scheme> -destination 'generic/platform=iOS' -configuration Release build CODE_SIGNING_ALLOWED=NO`. On failure: fix, re-run, same as any other 3c failure. Reference case (2026-09-05): the Debug gate passed 758/758 while the Release archive failed to compile on a DEBUG-only symbol referenced from a file that had just lost its `#if DEBUG` guard.

...
**Orchestrator-only refutation (no verifier dispatch).** A cross-ref or worker finding MAY be refuted directly by the orchestrator, skipping a `finding-verifier` dispatch, but only when ALL of these hold: (1) the refutation rests on a concrete call-site or definition the orchestrator actually read in this run (a `file:line` of the real call, never inference from naming or the finding's own description), (2) that evidence is written into the log's `Result` field so a reader can check it without re-running anything, (3) `patterns-store.sh dismissed {pattern}` is still called, same as any other `REFUTED` verdict. Worked example: 2026-09-05, a finding claimed the music cover renders at 200pt in a 56pt slot; refuted because `SignalCard.swift:101` never passes `prominent:` while `SignalDetailSheet.swift:58` and `RecapStory.swift:793` do — the 200pt path is never reached from the card the finding named. Anything short of a read call site (a plausible-sounding argument, a memory of similar code elsewhere) still goes through D.7 normally.
